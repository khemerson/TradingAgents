...
def test_microcap_position_too_large():
    """Micro-cap: 10% position rejected (max 5%)."""
    d = {"action": "BUY", "ticker": "RAVE-USD", "entry_price": 4.93,
         "stop_loss": 4.0, "take_profit": 10.0, "position_size_pct": 10, "confidence": 7}
    r = enforce(d, 10000, 90, is_microcap=True)
    assert not r.valid, f"Expected invalid, got {r}"
    assert any("POSITION" in v for v in r.violations), f"Expected POSITION violation, got {r.violations}"
    print("  PASS test_microcap_position_too_large")

def test_microcap_position_ok():
    """Micro-cap: 4% position accepted."""
    d = {"action": "BUY", "ticker": "RAVE-USD", "entry_price": 4.93,
         "stop_loss": 4.2, "take_profit": 10.0, "position_size_pct": 4, "confidence": 7}
    r = enforce(d, 10000, 90, is_microcap=True)
    assert r.valid, f"Expected valid, got violations: {r.violations}"
    print("  PASS test_microcap_position_ok")

def test_microcap_low_confidence():
    """Micro-cap: confidence 5 rejected (min 6)."""
    d = {"action": "BUY", "ticker": "RAVE-USD", "entry_price": 4.93,
         "stop_loss": 4.2, "take_profit": 10.0, "position_size_pct": 3, "confidence": 5}
    r = enforce(d, 10000, 90, is_microcap=True)
    assert not r.valid, f"Expected invalid, got {r}"
    assert any("CONFIDENCE" in v for v in r.violations), f"Expected CONFIDENCE violation, got {r.violations}"
    print("  PASS test_microcap_low_confidence")

def test_non_microcap_position_ok():
    """Non micro-cap: 10% position accepted (max 30%)."""
    d = {"action": "BUY", "ticker": "BTC-USD", "entry_price": 80000,
         "stop_loss": 75000, "take_profit": 90000, "position_size_pct": 10, "confidence": 7}
    r = enforce(d, 10000, 90, is_microcap=False)
    assert r.valid, f"Expected valid, got violations: {r.violations}"
    print("  PASS test_non_microcap_position_ok")

def test_microcap_stoploss_wide_ok():
    """Micro-cap: SL -12% accepted (max -15% for micro-cap vs -7% normal)."""
    d = {"action": "BUY", "ticker": "RAVE-USD", "entry_price": 5.0,
         "stop_loss": 4.4, "take_profit": 10.0, "position_size_pct": 4, "confidence": 7}
    r = enforce(d, 10000, 90, is_microcap=True)
    assert r.valid, f"Expected valid (SL -12% within micro-cap -15% limit), got violations: {r.violations}"
    print("  PASS test_microcap_stoploss_wide_ok")


# Run micro-cap tests
if __name__ == "__main__":
    test_microcap_position_too_large()
    test_microcap_position_ok()
    test_microcap_low_confidence()
    test_non_microcap_position_ok()
    test_microcap_stoploss_wide_ok()
