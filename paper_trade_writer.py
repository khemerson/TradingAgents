...
def _log_writer_refusal(payload: dict[str, Any]) -> None:
    """Append one JSONL line to data/writer_refused_YYYYMMDD.jsonl. Never raises."""
    try:
        REFUSED_LOG_DIR.mkdir(parents=True, exist_ok=True)
        day = datetime.now(timezone.utc).strftime("%Y%m%d")
        log_path = REFUSED_LOG_DIR / f"writer_refused_{day}.jsonl"
        with open(log_path, "a") as f:
            f.write(json.dumps(payload, ensure_ascii=False, default=str) + "\n")
    except Exception as exc:  # noqa: BLE001
        print(f"[writer] failed to write refusal log: {exc}")
...
def _get_spot_price(ticker: str) -> float | None:
    """Fetch current market price. Binance for crypto, yfinance for stocks."""
    upper = ticker.upper()
    # Crypto: use Binance
    symbol = _TICKER_MAP.get(upper) or upper.replace("-USD", "USDT").replace("-", "")
...
def record_decision(
    ...
    llm_price = float(decision.get("entry_price", 0))
    if llm_price <= 0:
        return {"status": "skipped", "reason": "No valid entry_price", "symbol": symbol}

    # Spot is authoritative. F6+ writer-side guard: if the LLM's declared entry
    # diverges from spot by more than DIVERGENCE_REJECT_THRESHOLD, the
    # reference-price prompt header was ignored upstream — hard reject.
    spot_price = _get_spot_price(ticker)
    if spot_price and spot_price > 0:
        divergence = abs(spot_price - llm_price) / llm_price
        if divergence > DIVERGENCE_REJECT_THRESHOLD:
            payload = {
                ...
                "threshold": DIVERGENCE_REJECT_THRESHOLD,
                "reason": "residual_price_divergence",
                "analysis_id": analysis_id,
                "action": action,
            }
            _log_writer_refusal(payload)
            print(f"[writer] 🚫 REJECT {ticker}: spot {spot_price:.4f} vs LLM {llm_price:.4f} "
                  f"= {divergence * 100:.1f}% divergence (threshold {DIVERGENCE_REJECT_THRESHOLD * 100:.0f}%)")
            return {
                "status": "rejected",
                "reason": "residual_price_divergence",
                "symbol": symbol,
        ...
        entry_price = llm_price
        print(f"[writer] \u26a0\ufe0f Could not fetch spot price for {ticker}, using LLM price {llm_price}")
    ...
    try:
        ...
        print(f"[writer] DB ERROR for {symbol}: {e}")
        print(f"[writer]    Decision {action} @ {entry_price} NOT persisted — pipeline continues")
        return {"status": "db_error", "reason": str(e), "symbol": symbol}
